...
void interpret(string const& _source, bool _inspect, bool _enableExternalCalls)
{
	...
	try
	{
		...
		if (_inspect)
			InspectedInterpreter::run(std::make_shared<Inspector>(_source, state), state, dialect, *ast, _enableExternalCalls, /*disableMemoryTracing=*/false);

		else
			Interpreter::run(state, dialect, *ast, _enableExternalCalls, /*disableMemoryTracing=*/false);
	}
